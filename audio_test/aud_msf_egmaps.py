...
def get_df(pro_dir, mrs_dir, class_name):
    ...
    full = pd.DataFrame(columns = list(range(223 + req_len - 2)) + ['class'])
    ...
    for f in tqdm(common_list):
        ...
        pro_df = pd.read_csv(pro_curr, sep = ',', header = None)
...
def wrapper(pro_grand, mrs_grand, set_name, noise):
    pro_anger = pro_grand + set_name + '_anger_' + noise + '/msf/'
    mrs_anger = mrs_grand + set_name + '_anger_' + noise + '/'
    
    pro_sad = pro_grand + set_name + '_sad_' + noise + '/msf/'
...
pro_grand = '../../../mitacs/MELD_dataset_MSF/'
mrs_grand = '../../../mitacs/MELD_noise_eGEMAPS_feat/'

noise = 'babble_20dB'
name = 'msf_gmap_' + noise
