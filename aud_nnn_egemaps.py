...
def get_df(par_dir, class_name):
    ...
    df = pd.read_csv(file_list[0], sep = ';', header = None, skiprows = [0])

    full = pd.DataFrame(columns = list(range(len(df.columns) - 2)) + ['class'])
    
#     print(full)
    i = 0
    for f in tqdm(file_list):
        df = pd.read_csv(f, sep = ';', header = None, skiprows = [0])
...
def wrapper(grandpa_dir, set_name, noise):
    anger = grandpa_dir + set_name + '_anger_' + noise + '/'
    sad = grandpa_dir + set_name + '_sad_' + noise + '/'    
...
grandpa_dir = '../../mitacs/MELD_noise_eGEMAPS_feat/'
noise = 'babble_0dB'
...
nnn = Models.NormalNeuralNetwork(0.2, class_names, (X_train.shape[1], ))
